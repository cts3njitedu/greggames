...
public class SaveGamePostProcessor extends AbstractPostProcessor {
	...
	private synchronized SpadeGame saveGame(SpadeGame spadeGame) {
		...
		SpadeGame newSpadeGame = spadePersistenceDal.findGame(spadeGame.getGameId());
		mergeNewWithOld(spadeGame, newSpadeGame);
		newSpadeGame.setLock(false);
		if(spadeNotifications.contains(newSpadeGame.getGameNotification())) {
			newSpadeGame.setMaxTime(maxTime);
		}
		return spadePersistenceDal.saveGame(newSpadeGame);
		...
		for(Entry<TeamTable, SpadeTeam> teamEntry: newSpadeGame.getTeams().entrySet()) {
			...
			for(Entry<PlayerTable, SpadePlayer>playerEntry:players.entrySet()) {
				if(playerEntry.getKey()!=newSpadeGame.getGameModifier()) {
					players.put(playerEntry.getKey(), 
							oldTeams.get(teamEntry.getKey()).getPlayers().get(playerEntry.getKey()));
				}
			}
		}
	}
